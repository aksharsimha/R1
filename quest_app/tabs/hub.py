...
def render(user_info):
    ...
    st.markdown("""
    <style>
    /* Total reset & Hub layout styling */
    ...
    div[data-testid="stColumn"]:nth-of-type(1) div.stButton > button[kind="primary"]:hover {
        box-shadow: 0 6px 25px rgba(168, 85, 247, 0.65) !important;
    ...
    div[data-testid="stColumn"]:nth-of-type(2) div.stButton > button[kind="primary"]:hover {
        box-shadow: 0 6px 25px rgba(59, 130, 246, 0.65) !important;
        transform: translateY(-1px) !important;
    }
    </style>
    ...
    with card_col1:
        ...
        <svg width="150" height="90" viewBox="0 0 150 90" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <!-- Isometric Bars -->
            <rect x="25" y="60" width="14" height="24" rx="3" fill="url(#barGrad1)" />
            <rect x="45" y="48" width="14" height="36" rx="3" fill="url(#barGrad1)" />
            <rect x="65" y="38" width="14" height="46" rx="3" fill="url(#barGrad2)" />
            <rect x="85" y="26" width="14" height="58" rx="3" fill="url(#barGrad2)" />
            <rect x="105" y="14" width="14" height="70" rx="3" fill="url(#barGrad3)" />
            
            <!-- Glowing Ascending Trend Line & Arrow -->
            <path d="M15 68 Q 50 54 80 32 T 128 10" fill="none" stroke="url(#arrowGrad)" stroke-width="3.5" stroke-linecap="round" filter="url(#glowEffect)"/>
            <polygon points="122,8 136,8 132,22" fill="#e879f9" filter="url(#glowEffect)"/>
        </svg>
        """
    ...
    with card_col2:
        html_edu_card = f"""
        <div class="hub-card-box edu-theme">
            <div>
                <div class="hub-card-top">
                    <div class="hub-card-header-left">
    ...
    with bot_c1:
        st.markdown(f"""
        <div style="background: rgba(13, 15, 28, 0.75); backdrop-filter: blur(16px); border: 1px solid rgba(255, 255, 255, 0.07); border-radius: 16px 0 0 16px; padding: 1.1rem 1.6rem; display: flex; align-items: center; justify-content: space-between; box-shadow: 0 8px 30px rgba(0, 0, 0, 0.5); min-height: 70px;">
    ...
    with bot_c2:
        st.markdown("""
        <div style="background: rgba(13, 15, 28, 0.75); backdrop-filter: blur(16px); border: 1px solid rgba(255, 255, 255, 0.07); border-left: none; border-radius: 0 16px 16px 0; padding: 1.1rem 1.2rem; display: flex; align-items: center; justify-content: center; box-shadow: 0 8px 30px rgba(0, 0, 0, 0.5); min-height: 70px;">
        """, unsafe_allow_html=True)
