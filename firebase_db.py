...
# ──────────────────────────────────────────────────────────────────────────────
# Profile Customization (PRO System)
# ──────────────────────────────────────────────────────────────────────────────
...
def get_profile_customization(username: str) -> dict:
    """Retrieve profile customization settings, returning defaults for missing fields."""
    if not username or not username.strip():
        return dict(_DEFAULT_CUSTOMIZATION)
    try:
        profile = get_user_profile(username)
        stored = profile.get("profile_customization", {})
        if not isinstance(stored, dict):
            stored = {}
        # Merge with defaults so new fields are always present
        result = dict(_DEFAULT_CUSTOMIZATION)
...
def save_profile_customization(username: str, customization: dict) -> bool:
    """Persist profile customization settings to the user's Firestore document."""
    if not username or not username.strip():
        return False
    try:
        db = get_db()
        # Only save recognized keys to prevent injection of arbitrary fields
        clean = {}
        for key in _DEFAULT_CUSTOMIZATION:
            if key in customization:
                clean[key] = customization[key]
        db.collection("users").document(username).update({
            "profile_customization": clean,
        })
        return True
    except Exception as e:
        print(f"[firebase_db] Failed to save profile customization: {e}")
        return False


def set_pro_status(username: str, is_pro: bool) -> bool:
    """Admin toggle for PRO subscription status."""
    if not username or not username.strip():
        return False
    try:
        db = get_db()
